...
void
scheduleMLFQ(struct cpu *c, struct proc *proc){

  struct proc *p, *next_proc;
  int temp_queno = 0;

  for(p = proc ; p < &proc[NPROC] ; p++){
    if(p->inqueue > 0 && p->cwtime >= 100){
      if(p->queno > 0){
        int queno = p->queno;
        Pop(p, queno);
        Enqueue(p, queno - 1);
  ...
  for(p = proc ; p < &proc[NPROC] ; p++){
    if(p->inqueue == 0 && p->state == RUNNABLE){
      if(p->queno != 4){
        temp_queno = p->queno;
      }
      Enqueue(p, temp_queno);
    }
  }
